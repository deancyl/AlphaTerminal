...
class TestPerformanceBenchmarks:
    ...
    async def test_treemap_build_benchmark(self):
        ...
        stocks = [{"symbol": f"{600000+i}", "name": f"股票{i}",
                   "market_cap": 1e10 + i * 1e8, "change_pct": i % 10 - 5}
                  for i in range(500)]
        sector_stocks = {f"板块{i}": [{"symbol": f"{600000+i}", "name": f"股票{i}",
                                        "market_cap": 1e10} for i in range(20)]
                         for i in range(30)}

        with patch('app.services.market_radar.treemap_builder._fetch_sectors', return_value=sectors):
            with patch('app.services.market_radar.treemap_builder._fetch_all_stocks', return_value=stocks):
                with patch('app.services.market_radar.treemap_builder._fetch_sector_stocks_batch', return_value=sector_stocks):
...
class TestConcurrentStress:
    ...
    async def test_concurrent_treemap_requests(self):
        ...
        with patch('app.services.market_radar.treemap_builder._fetch_sectors', return_value=sectors):
            with patch('app.services.market_radar.treemap_builder._fetch_all_stocks', return_value=stocks):
                with patch('app.services.market_radar.treemap_builder._fetch_sector_stocks_batch', return_value={}):

                    tasks = [build_treemap_data(level="sector", timeout=15.0) for _ in range(100)]

                    start = time.perf_counter()
                    results = await asyncio.gather(*tasks, return_exceptions=True)
                    elapsed = time.perf_counter() - start

                    success_count = sum(1 for r in results if not isinstance(r, Exception) and "data" in r)
...
class TestRateLimitUnderLoad:
    ...
    def test_rate_limit_config_under_load(self):
        ...
        paths = ["/api/v1/market_radar/treemap", "/api/v1/market_radar/anomalies",
                 "/api/v1/market_radar/health"] * 100
