...
def temp_strategy_db():
    ...
    """)
    test_conn.execute("CREATE INDEX IF NOT EXISTS idx_strategies_name ON strategies(name)")
    test_conn.execute("CREATE INDEX IF NOT EXISTS idx_strategies_market ON strategies(market)")
    test_conn.execute("CREATE INDEX IF NOT EXISTS idx_strategies_deleted ON strategies(deleted_at)")
...
class TestStrategyValidationEndpoint:
    ...
    def test_validate_success(self, temp_strategy_db, sample_strategy_code):
        ...
        response = temp_strategy_db['client'].post(
            ...
            json={"code": sample_strategy_code}
        )
    ...
    def test_validate_detects_forbidden_imports(self, temp_strategy_db, malicious_code_import):
        """Test validate detects forbidden imports."""
        response = temp_strategy_db['client'].post(
            "/api/v1/strategy/validate",
            json={"code": malicious_code_import}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_validate_detects_forbidden_functions(self, temp_strategy_db, malicious_code_eval):
        """Test validate detects forbidden function calls."""
        response = temp_strategy_db['client'].post(
            "/api/v1/strategy/validate",
            json={"code": malicious_code_eval}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_valid"] is False
    ...
    def test_validate_detects_infinite_loops(self, temp_strategy_db, infinite_loop_code):
        """Test validate detects infinite loops."""
        response = temp_strategy_db['client'].post(
            "/api/v1/strategy/validate",
            json={"code": infinite_loop_code}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_valid"] is False
    ...
    def test_validate_syntax_error(self, temp_strategy_db):
        ...
        response = temp_strategy_db['client'].post(
            "/api/v1/strategy/validate",
            json={"code": "def foo(:"}  # Invalid syntax
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_valid"] is False
...
class TestBacktestRunEndpoint:
    ...
    def test_backtest_run_success(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_backtest_run_response_structure(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_backtest_run_date_validation(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_backtest_run_initial_capital_validation(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_backtest_run_commission_validation(self, temp_strategy_db, sample_strategy_code, mock_api_key):
...
class TestStrategyOptimizeEndpoint:
    ...
    def test_optimize_success(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_optimize_response_structure(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_optimize_param_grid_validation(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_optimize_metric_validation(self, temp_strategy_db, sample_strategy_code, mock_api_key):
...
class TestStrategyCRUDEndpoints:
    ...
    def test_create_strategy_success(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_get_strategy_success(self, temp_strategy_db, sample_strategy_code, mock_api_key):
        ...
        with patch('app.middleware.require_api_key', return_value=None):
            with patch.object(strategy_db, '_get_conn', return_value=mock_conn):
                with patch.object(strategy_db, '_lock', MagicMock()):
                    response = temp_strategy_db['client'].get(f"/api/v1/strategy/strategies/{strategy_id}")
                    assert response.status_code == 200
                    data = response.json()
                    assert data["code"] == 0
    ...
    def test_delete_strategy_success(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_get_strategy_not_found(self, temp_strategy_db):
        ...
        response = temp_strategy_db['client'].get("/api/v1/strategy/strategies/non-existent-id")
...
class TestStrategyInputValidation:
    ...
    def test_backtest_symbol_required(self, temp_strategy_db, sample_strategy_code, mock_api_key):
    ...
    def test_backtest_date_format_validation(self, temp_strategy_db, sample_strategy_code, mock_api_key):
        """Test backtest validates date format."""
        with patch('app.middleware.require_api_key', return_value=None):
            response = temp_strategy_db['client'].post(
                "/api/v1/strategy/backtest",
    ...
    def test_optimize_param_grid_required(self, temp_strategy_db, sample_strategy_code, mock_api_key):
...
class TestStrategyErrorHandling:
    ...
    def test_backtest_data_fetch_error(self, temp_strategy_db, sample_strategy_code, mock_api_key):
