...
class TestAuditVerify:
    ...
    def test_verify_chain_with_range(self, temp_audit_db):
        """测试带范围参数的链验证"""
        response = temp_audit_db["client"].get(
            "/api/v1/audit/verify?from_id=1&to_id=50"
        )
        assert response.status_code == 200
        data = response.json()
        assert "valid" in data
...
class TestAuditLogs:
    ...
    def test_logs_with_cursor(self, temp_audit_db):
        ...
        response = temp_audit_db["client"].get(
            "/api/v1/audit/logs?after_timestamp=2024-01-01T00:00:00"
        )
        assert response.status_code == 200
        data = response.json()
...
class TestAuditValidation:
    ...
    def test_logs_after_timestamp_invalid(self, temp_audit_db):
        ...
        response = temp_audit_db["client"].get(
            "/api/v1/audit/logs?after_timestamp=invalid"
        )
