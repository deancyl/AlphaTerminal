...
def _create_mock_akshare(spot_df=None, history_df=None, cfets_df=None, boc_df=None,
                         spot_error=None, cfets_error=None, boc_error=None):
...
class TestForexFetcherSpot:
    ...
    async def test_fetch_spot_with_cross_pairs(self, sample_spot_dataframe):
        ...
        cross_df = pd.concat([sample_spot_dataframe, pd.DataFrame({
            '代码': ['EURUSD', 'GBPUSD'],
            '名称': ['欧元/美元', '英镑/美元'],
            '最新价': [1.0850, 1.2650],
            '涨跌额': [0.0010, 0.0020],
...
class TestForexFetcherFallback:
    ...
    async def test_fallback_to_boc_on_cfets_failure(self, sample_boc_dataframe):
        ...
        with patch('app.services.fetchers.forex_fetcher._get_akshare', return_value=mock_ak):
            result = await fetcher.get_spot_quotes()
    ...
    async def test_fallback_preserves_quote_structure(self):
        ...
        required_fields = ['symbol', 'name', 'latest', 'bid', 'ask', 'spread',
                          'change', 'change_pct', 'open', 'high', 'low',
                          'prev_close', 'source', 'is_demo', 'timestamp']
    ...
    async def test_fallback_logs_warning(self, caplog):
        ...
        assert any("失败" in record.message or "error" in record.message.lower()
