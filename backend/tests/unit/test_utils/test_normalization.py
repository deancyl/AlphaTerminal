...
def test_normalization():
    ...
    for symbol, data in mock_alpha_vantage_data.items():
        ...
        for i, day in enumerate(data):
            ...
            print(
                f"     原始: O={open_:.2f} H={high:.2f} L={low:.2f} C={close:.2f} V={vol:.0f}"
            )
            print(
                f"     标准化: O={norm_open:.2f} H={norm_high:.2f} L={norm_low:.2f} C={norm_close:.2f} V={norm_vol:.0f}"
            )
    ...
    try:
        ...
        response = urllib.request.urlopen(
            "http://localhost:8002/api/v1/market/history/000001?period=daily&limit=5"
        )
        ...
        for i, day in enumerate(sh_data[:3]):
            print(
                f"  {i+1}. {day['date']}: O={day['open']:.2f} H={day['high']:.2f} L={day['low']:.2f} C={day['close']:.2f}"
            )
