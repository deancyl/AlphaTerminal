...
class TestBacktestEngine:
    ...
    def config(self):
        ...
        return BacktestConfig(
            initial_capital=100000, commission=0.0003, slippage=0.0001, warmup_bars=10
        )
    ...
    def test_engine_initialization(self, config):
        ...
        assert any(
            "DEBUG CYCLE 1: ENGINE INITIALIZATION" in log for log in engine._debug_log
        )
    ...
    def test_strategy_compilation(self, config, sample_data):
        ...
        assert any(
            "DEBUG CYCLE 2: STRATEGY COMPILATION" in log for log in result.debug_log
        )
    ...
    def test_metric_calculation(self, config, sample_data):
        ...
        assert any(
            "DEBUG CYCLE 7: METRIC CALCULATION" in log for log in result.debug_log
        )
    ...
    def test_result_generation(self, config, sample_data):
        ...
        assert any(
            "DEBUG CYCLE 8: RESULT GENERATION" in log for log in result.debug_log
        )
    ...
    def test_performance_summary(self, config, sample_data):
        ...
        assert any(
            "DEBUG CYCLE 10: PERFORMANCE SUMMARY" in log for log in result.debug_log
        )
    ...
    def test_commission_deduction(self, config, sample_data):
        ...
        if result.orders:
            ...
            expected_commission = (
                order.filled_price * order.filled_quantity * config.commission
            )
    ...
    def test_stop_loss(self, sample_data):
        ...
        config = BacktestConfig(
            initial_capital=100000, stop_loss_pct=5.0, warmup_bars=10
        )
        engine = BacktestEngine(config)
    ...
    def test_take_profit(self, sample_data):
        ...
        config = BacktestConfig(
            initial_capital=100000, take_profit_pct=10.0, warmup_bars=10
        )
        engine = BacktestEngine(config)
...
class TestSampleStrategy:
    ...
    def test_ma_crossover_strategy(self, trending_data):
        ...
        config = BacktestConfig(
            initial_capital=100000, commission=0.0003, warmup_bars=30
        )
        engine = BacktestEngine(config)
    ...
    def test_verify_all_metrics_calculated(self, trending_data):
        ...
        config = BacktestConfig(initial_capital=100000, warmup_bars=30)
        engine = BacktestEngine(config)
