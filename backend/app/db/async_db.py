...
_db_executor: ThreadPoolExecutor = ThreadPoolExecutor(
    max_workers=50, thread_name_prefix="db_worker"
)
...
def async_wrap(
    ...
    async def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        future = loop.run_in_executor(
            exec_pool, functools.partial(sync_func, *args, **kwargs)
        )
...
async_update_session_activity = async_wrap(
    update_session_activity, timeout=DEFAULT_DB_TIMEOUT
)
...
async_get_all_model_configs = async_wrap(
    get_all_model_configs, timeout=DEFAULT_DB_TIMEOUT
)
