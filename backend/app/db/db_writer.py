...
def _write_daily(conn, rows):
    ...
    for i in rows:
        try:
            ...
            fail += 1
            logger.error(
                f"[DBWriter] daily insert failed: symbol={i.get('symbol')}, error={type(e).__name__}: {e}",
                exc_info=True,
            )
    conn.commit()
    return ok, fail
...
def _write_periodic(conn, rows, period):
    ...
    for i in rows:
        try:
            ...
            logger.error(
                f"[DBWriter] periodic insert failed: {type(e).__name__}: {e}",
                exc_info=True,
            )
    conn.commit()
    return ok, 0
...
def _flush_realtime(conn):
    ...
    for r in rows:
        try:
            ...
            error_count += 1
            logger.error(
                f"[DBWriter] realtime insert error: {type(e).__name__}: {e}",
                exc_info=True,
            )
    if processed_keys:
        ...
        conn.execute(
            f"DELETE FROM write_buffer WHERE (symbol, name) IN (VALUES {placeholders})",
            flat,
        )
    conn.commit()
        ...
        try:
            ...
            """,
                (
                    ...
                    (
                        float(r.get("per") or 0)
                        if r.get("per") not in (None, "")
                        else None
                    ),
            ...
            logger.error(
                f"[DBWriter] all_stocks upsert failed: {type(e).__name__}: {e}",
                exc_info=True,
            )
    conn.commit()
    return ok, 0
...
def _write_buffer(conn, rows):
    ...
    for item in rows:
        try:
            conn.execute(
                "INSERT INTO write_buffer VALUES (?,?,?)",
                (item.get("symbol", ""), item.get("name", ""), json.dumps(item)),
            )
            ok += 1
        except (sqlite3.Error, ValueError, TypeError) as e:
            logger.error(
                f"[DBWriter] buffer insert failed: {type(e).__name__}: {e}",
                exc_info=True,
            )
    conn.commit()
    return ok, 0
...
def _write_cache_persist(conn, rows):
    ...
    for item in rows:
        try:
            ...
            logger.error(
                f"[DBWriter] cache_persist insert failed: key={item.get('key')}, {type(e).__name__}: {e}",
                exc_info=True,
            )
    conn.commit()
    return ok, 0
...
def _write_fund_nav(conn, rows):
    ...
    for item in rows:
        try:
            ...
            logger.error(
                f"[DBWriter] fund_nav insert failed: fund_code={item.get('fund_code')}, {type(e).__name__}: {e}",
                exc_info=True,
            )
    conn.commit()
    return ok, 0
        ...
        while True:
            ...
            try:
                if task_type == T_DAILY:
                    ...
                    logger.info(
                        f"[DBWriter] ✅ realtime flush {ok_cnt} rows"
                        + (f", {err_cnt} failed" if err_cnt else "")
                    )
...
def enqueue(task: dict):
    ...
    logger.debug(
        f"[DBQueue] enqueued {task.get('type')} ({len(task.get('rows', []))} rows), queue_size={qsize}"
    )
...
def start_writer():
    ...
    if _writer_thread is None or not _writer_thread.is_alive():
        ...
        _writer_thread = threading.Thread(
            target=db_writer_loop, name="DBWriter", daemon=True
        )
...
def stop_writer():
    ...
    if _writer_thread and _writer_thread.is_alive():
        ...
        if remaining > 0:
            logger.info(
                f"[DBWriter] waiting for {remaining} tasks to flush (max 30s)..."
            )
        _writer_thread.join(timeout=30)
        if _writer_thread.is_alive():
            logger.warning(
                f"[DBWriter] ⚠️  {remaining} tasks still in queue after 30s timeout"
            )
        else:
