...
def get_session(session_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {
            ...
            "bound_models": (
                json.loads(row["bound_models"]) if row["bound_models"] else []
            ),
...
def extend_session(session_id: str, additional_hours: int = 24) -> bool:
    with _session_lock:
        ...
        try:
            row = conn.execute(
                "SELECT expires_at FROM copilot_sessions WHERE session_id = ?",
                (session_id,),
            ).fetchone()
            ...
            new_expires = max(datetime.now(), current_expires) + timedelta(
...
def get_session_conversations(
    ...
    try:
        ...
        logger.warning(
            f"[SessionDB] get_session_conversations error: {e}", exc_info=True
        )
        return []
    finally:
        conn.close()
...
def cleanup_expired_admin_sessions() -> int:
    with _session_lock:
        ...
        try:
            ...
            if deleted_count > 0:
                logger.info(
                    f"[AdminSession] Cleaned up {deleted_count} expired sessions"
                )
            return deleted_count
        finally:
            conn.close()
