...
BUILTIN_MODELS = [
    ...
    },
    {
        ...
        "context_length": 64000,
        "metadata": json.dumps(
            {"family": "deepseek", "reasoning": True, "cache_hit_discount": 0.1}
        ),
    },
    # Qianwen Models
    {
...
def seed_pricing_catalog(
    models: List[Dict[str, Any]] = None, force: bool = False
) -> Dict[str, int]:
    if models is None:
    ...
    try:
        ...
        logger.info(
            f"[Seed] Pricing catalog: {inserted} inserted, {updated} updated, {skipped} skipped"
        )
...
def get_pricing_by_model(model_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        row = conn.execute(
            "SELECT * FROM model_pricing_catalog WHERE model_id = ?", (model_id,)
        ).fetchone()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force update existing entries"
    )
    ...
    if args.list:
        ...
        for p in pricing:
            ...
            print(
                f"{builtin} {p['model_id']}: ${p['input_cost_per_token']*1e6:.2f}/1M in, ${p['output_cost_per_token']*1e6:.2f}/1M out"
            )
    else:
