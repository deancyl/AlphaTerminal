...
def get_schema_conn():
    ...
    try:
        ...
        logger.error(
            f"[Schema] Database error during schema operation: {type(e).__name__}: {e}",
            exc_info=True,
        )
        try:
            conn.rollback()
            ...
            logger.error(
                f"[Schema] Rollback failed: {type(rollback_err).__name__}: {rollback_err}",
                exc_info=True,
            )
        raise
    except Exception as e:
        logger.error(
            f"[Schema] Unexpected error during schema operation: {type(e).__name__}: {e}",
            exc_info=True,
        )
        try:
            conn.rollback()
...
def create_model_pricing_catalog_table(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_pricing_provider ON model_pricing_catalog(provider)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_pricing_builtin ON model_pricing_catalog(is_builtin)"
    )
...
def create_token_usage_logs_table(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_usage_session ON token_usage_logs(session_id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_usage_model ON token_usage_logs(model_id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_usage_provider ON token_usage_logs(provider)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_usage_user ON token_usage_logs(user_id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_usage_created ON token_usage_logs(created_at)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_usage_session_created ON token_usage_logs(session_id, created_at)"
    )
...
def create_copilot_sessions_table(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_session_user ON copilot_sessions(user_id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_session_active ON copilot_sessions(last_active_at)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_session_expires ON copilot_sessions(expires_at)"
    )
...
def create_model_config_versions_table(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_config_version_created ON model_config_versions(created_at)"
    )
...
def create_usage_aggregates_table(conn):
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_agg_type ON usage_aggregates(aggregate_type)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_agg_key ON usage_aggregates(aggregate_key)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_agg_model ON usage_aggregates(model_id)"
    )
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_agg_created ON usage_aggregates(created_at)"
    )
