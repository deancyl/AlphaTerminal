...
def add_model_to_config(
    provider_key: str, model_id: str, model_config: Dict[str, Any]
) -> bool:
    config = get_model_config(provider_key)
    if not config:
...
def update_model_in_config(
    provider_key: str, model_id: str, updates: Dict[str, Any]
) -> bool:
    config = get_model_config(provider_key)
...
def save_config_version(
    config_snapshot: Dict[str, Any], created_by: str = None, change_summary: str = None
) -> int:
    conn = _get_conn()
    try:
        cursor = conn.execute(
