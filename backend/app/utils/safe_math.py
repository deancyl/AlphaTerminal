...
def safe_divide(
    dividend: Union[int, float], divisor: Union[int, float], default_value: float = 0.0
) -> float:
    """
    ...
    if (
        divisor is None
        or divisor == 0
        or (isinstance(divisor, float) and (math.isnan(divisor) or math.isinf(divisor)))
    ):
...
def safe_percent(
    part: Union[int, float], total: Union[int, float], default_value: float = 0.0
) -> float:
    """
...
def safe_average(
    values: Optional[List[Union[int, float]]], default_value: float = 0.0
) -> float:
    """
...
def safe_round(
    value: Union[int, float], decimals: int = 2, default_value: float = 0.0
) -> float:
    """
