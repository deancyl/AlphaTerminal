...
from ..services.agent.token_service import AgentToken, TokenScope, get_token_service
...
def get_current_token(request: Request) -> Optional[AgentToken]:
    ...
    logger.debug("[AUTH_TOKEN_GET] Getting current token from request")
    ...
    if token:
        ...
        logger.debug("[AUTH_TOKEN_GET] No token in context")
...
async def verify_agent_token(
    ...
    logger.debug("[AUTH_VERIFY] Token service instance obtained")
    ...
    logger.debug("[AUTH_VERIFY] Rate limit check passed")
    ...
    logger.debug("[AUTH_VERIFY] Context injected into request state")
