...
async def market_stocks(
    ...
    sort_by: str = Query(
        "chg_pct", description="排序字段：chg_pct|turnover|price|code"
    ),
    ...
    try:
        return success_response(
            query_stocks(page=page, page_size=page_size, sort_by=sort_by, asc=asc)
        )
    except Exception as e:
...
async def debug_trigger():
    ...
    return success_response(
        {
            "status": "triggered",
            "message": "已触发后台刷新，请稍后查看 /debug/scheduler",
        }
    )
    ...
    try:
        ...
        async def fetch_with_timeout():
            try:
                ...
                logger.warning(
                    f"[FundFlow] fetch timeout after {FETCH_TIMEOUT}s", exc_info=True
                )
                return None
    ...
    try:
        ...
        async def fetch_with_timeout():
            try:
                ...
                logger.warning(
                    f"[IndustryFundFlow] fetch timeout after {FETCH_TIMEOUT}s",
                    exc_info=True,
                )
                return None
