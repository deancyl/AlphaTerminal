...
async def list_positions(
    ...
    limit: int = Query(
        50, ge=1, le=500, description="Maximum number of positions to return"
    ),
                ...
                positions = [
                    {
                        ...
                        "shares": r[2],
                        "avg_cost": r[3],
                        "marketValue": r[4] or (r[2] * r[3] if r[2] and r[3] else 0),
                        "unrealized_pnl": r[5] or 0,
                        "cost": r[2] * r[3] if r[2] and r[3] else 0,
                        "updated_at": r[6],
                        "portfolio_name": r[7],
                        "portfolio_id": r[0],
                    }
    ...
    try:
        ...
        logger.warning(
            "[positions] list_positions timeout after %ds",
            PORTFOLIO_TIMEOUT,
            exc_info=True,
        )
...
async def upsert_position(body: PositionIn, _: None = Depends(require_api_key)):
    ...
    try:
        ...
        logger.warning(
            "[positions] upsert_position timeout after %ds",
            PORTFOLIO_TIMEOUT,
            exc_info=True,
        )
...
async def delete_position(
    portfolio_id: int, symbol: str, _: None = Depends(require_api_key)
):
    """清仓指定标的 - 需认证"""
    ...
    try:
        ...
        logger.warning(
            "[positions] delete_position timeout after %ds",
            PORTFOLIO_TIMEOUT,
            exc_info=True,
        )
...
async def portfolio_pnl(
    portfolio_id: int,
    include_children: bool = Query(False, description="是否包含子账户盈亏"),
):
    """
    实时浮动盈亏计算（专业增强版）
    ...
    def _sync_work():
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"[Portfolio PnL] 数据库操作错误，无法获取股票元数据: {e}",
                    exc_info=True,
                )
            ...
            try:
                ...
                logger.warning(
                    f"[Portfolio PnL] 数据库操作错误，无法获取 cash_balance (portfolio_id={portfolio_id}): {e}",
                    exc_info=True,
                )
            except Exception as e:
                logger.warning(
                    f"[Portfolio PnL] 获取 cash_balance 失败 (portfolio_id={portfolio_id}): {e}",
                    exc_info=True,
                )
        finally:
            conn.close()
        ...
        if not spot or len(spot) < 10:
            ...
            try:
                ...
                logger.warning(
                    f"[Portfolio PnL] 数据库操作错误，兜底获取价格失败: {e}",
                    exc_info=True,
                )
        ...
        for row in rows:
            ...
            if (
                sym_upper.startswith("6")
                or sym_upper.startswith("0")
                or sym_upper.startswith("3")
            ):
                market = "A股"
            ...
            pos_data = {
                ...
                "name": (
                    ...
                    if meta
                    else (info.get("name") if info else symbol)
                ),
                "shares": shares,
                ...
                "market": market,
                "pe": (
                    round(meta.get("per"), 2)
                    if meta and meta.get("per")
                    else (round(info.get("pe"), 2) if info and info.get("pe") else None)
                ),
                "pb": (
                    round(meta.get("pb"), 2)
                    if meta and meta.get("pb")
                    else (round(info.get("pb"), 2) if info and info.get("pb") else None)
                ),
                "turnover": (
                    round(meta.get("turnover"), 2)
                    if meta and meta.get("turnover")
                    else (
                        round(info.get("turnover"), 2)
                        if info and info.get("turnover")
                        else None
                    )
                ),
            }
        ...
        try:
            ...
            unrealized_pnl = (
                float(unrealized_row[0])
                if unrealized_row and unrealized_row[0] is not None
                else 0.0
            )
            ...
            logger.warning(
                f"[Portfolio PnL] 数据库操作错误，无法读取盈亏数据 (portfolio_id={portfolio_id}): {e}",
                exc_info=True,
            )
            realized_pnl = 0.0
            unrealized_pnl = 0.0
            ...
            logger.warning(
                f"[Portfolio PnL] 盈亏数据格式错误 (portfolio_id={portfolio_id}): {e}",
                exc_info=True,
            )
            realized_pnl = 0.0
            unrealized_pnl = 0.0
            ...
            logger.warning(
                f"[Portfolio PnL] 读取盈亏数据失败 (portfolio_id={portfolio_id}): {e}",
                exc_info=True,
            )
            realized_pnl = 0.0
            unrealized_pnl = 0.0
        ...
        response = {
            ...
            "total_pnl_pct": (
                round((total_value + cash_balance - total_cost) / total_cost * 100, 2)
                if total_cost
                else 0
            ),
            "daily_pnl": round(daily_pnl, 2),
            "daily_pnl_pct": (
                round(daily_pnl / total_cost * 100, 2) if total_cost else 0
            ),
        ...
        if include_children:
            ...
            response["portfolio_count"] = len(
                set(r.get("portfolio_id") for r in result if r.get("portfolio_id"))
            )
    ...
    try:
        ...
        logger.warning(
            "[positions] portfolio_pnl timeout after %ds",
            PORTFOLIO_TIMEOUT,
            exc_info=True,
        )
...
async def get_snapshots(
    ...
    try:
        ...
        logger.warning(
            "[positions] get_snapshots timeout after %ds",
            PORTFOLIO_TIMEOUT,
            exc_info=True,
        )
...
async def save_snapshot(portfolio_id: int, _: None = Depends(require_api_key)):
    ...
    try:
        ...
        logger.warning(
            "[positions] save_snapshot timeout after %ds",
            PORTFOLIO_TIMEOUT,
            exc_info=True,
        )
...
async def daily_pnl_only(portfolio_id: int):
    ...
    def _sync_work():
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"[Portfolio daily_pnl] 数据库操作错误，无法读取 unrealized (portfolio_id={portfolio_id}): {e}",
                    exc_info=True,
                )
            except Exception as e:
                logger.warning(
                    f"[Portfolio daily_pnl] 读取 unrealized 失败 (portfolio_id={portfolio_id}): {e}",
                    exc_info=True,
                )
    ...
    try:
        ...
        logger.warning(
            "[positions] daily_pnl timeout after %ds", PORTFOLIO_TIMEOUT, exc_info=True
        )
