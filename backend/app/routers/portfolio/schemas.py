...
class PortfolioIn(BaseModel):
    ...
    type: str = Field(
        default="portfolio", pattern="^(portfolio|account|strategy|group|main)$"
    )
    ...
    asset_class: str = Field(
        default="mixed", pattern="^(stock|bond|fund|futures|options|mixed)$"
    )
    strategy: Optional[str] = Field(
        default=None, pattern="^(value|growth|balanced|index|quant)$"
    )
...
class PortfolioUpdate(BaseModel):
    ...
    type: Optional[str] = Field(
        default=None, pattern="^(portfolio|account|strategy|group|main)$"
    )
    parent_id: Optional[int] = Field(default=None, ge=1)
    currency: Optional[str] = Field(default=None, pattern="^(CNY|USD|HKD|EUR)$")
    asset_class: Optional[str] = Field(
        default=None, pattern="^(stock|bond|fund|futures|options|mixed)$"
    )
    strategy: Optional[str] = Field(
        default=None, pattern="^(value|growth|balanced|index|quant)$"
    )
...
class TransactionIn(BaseModel):
    ...
    portfolio_id: int = Field(..., ge=1, description="账户ID")
    type: str = Field(
        ..., pattern="^(deposit|withdraw|transfer_in|transfer_out|dividend|fee)$"
    )
