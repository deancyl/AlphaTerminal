...
async def _fetch_curve_data_for_cache():
    ...
    try:
        ...
        if data and data.get("yield_curve"):
            ...
            logger.info(
                f"[Bond] Yield curve fetched from {source}, last_update: {last_update}"
            )
        ...
        _bond_cb.record_failure()
        logger.warning(
            f"[Bond] bond_fetcher failed: {type(e).__name__}: {e}", exc_info=True
        )
...
async def _fetch_history_df_for_cache():
    ...
    logger.info(
        "[Bond] _fetch_history_df_for_cache: fetching fresh data from akshare (cache miss)"
    )
    try:
        ...
        logger.warning(
            "[Bond] _fetch_history_df_for_cache timeout after 30s", exc_info=True
        )
        return None
...
async def bond_history(
    ...
    try:
        ...
        date_col = (
            "日期"
            if "日期" in df_gov.columns
            else (df_gov.columns[1] if len(df_gov.columns) > 1 else df_gov.columns[0])
        )
        ...
        tenor_col = next(
            (
                ...
                if c == tenor or c.startswith(tenor + "(") or c.startswith(tenor + "（")
            ),
            None,
        )
        if not tenor_col:
...
def _init_cache_warmup():
    ...
    async def warmup():
        try:
            ...
            if data and data.get("yield_curve"):
                logger.info(
                    f"[Bond] Cache warmup complete, source: {data.get('source')}"
                )
            else:
