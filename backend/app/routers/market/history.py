...
async def market_history(
    ...
    _NON_ASHARE = (
        ...
        or clean_sym.startswith("hk")
        or clean_sym.startswith("jp")
        or clean_sym.startswith("macro")
        or clean_sym
        in {
            "gold",
            "gld",
    ...
    if _NON_ASHARE and period in ("daily", "weekly", "monthly"):
        ...
        if (not raw_rows or (len(raw_rows) < 50 and offset == 0)) and offset == 0:
            ...
            try:
                ...
                if sync_rows:
                    ...
                    fetching = False
                    logger.info(
                        f"[Market History] 同步返回 {clean_sym}: {len(sync_rows)} 条"
                    )
            except asyncio.TimeoutError:
                logger.warning(
                    f"[Market History] 同步拉取超时 {clean_sym}，回退DB", exc_info=True
                )
            except Exception as e:
                logger.warning(
                    f"[Market History] 同步拉取失败，回退DB: {e}", exc_info=True
                )
        ...
        if clean_sym in MIN_KLINE_SUPPORTED:
            ...
            history = fetch_index_minute_history(
                clean_sym, limit=min(limit, 300), frequency=5, offset=offset
            )
        ...
        history = _inject_change_pct(
            _apply_adjustment(list(reversed(raw_rows)), adjustment)
        )
        has_more = (offset + len(raw_rows)) < total
        chart_type = "candlestick"

    elif period in ("weekly", "monthly"):
        raw_rows = get_periodic_history(
            clean_sym, period=period, limit=limit, offset=offset
        )
    ...
    try:
        ...
        logger.warning(f"[K-line Gap] Detection failed: {e}", exc_info=True)
...
async def futures_history(
    ...
    if period == "daily":
        try:
            ...
            df_work["volume"] = df_work["volume"].apply(
            ...
            logger.error(f"[Futures] daily timeout {clean_sym}", exc_info=True)
            return success_response(
                {"symbol": clean_sym, "period": period, "history": []}, "获取超时"
            )
        except Exception as e:
            logger.error(f"[Futures] daily failed {clean_sym}: {e}", exc_info=True)
            return success_response(
                {"symbol": clean_sym, "period": period, "history": []}, f"获取失败: {e}"
            )
        ...
        try:
            ...
            rows = df_work[
                ["date", "open", "high", "low", "close", "volume", "hold", "timestamp"]
            ].to_dict("records")
            return {"symbol": clean_sym, "period": period, "history": rows}
        except asyncio.TimeoutError:
            logger.error(f"[Futures] minute timeout {clean_sym}", exc_info=True)
            return success_response(
                {"symbol": clean_sym, "period": period, "history": []}, "获取超时"
            )
        except Exception as e:
