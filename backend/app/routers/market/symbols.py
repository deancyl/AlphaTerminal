...
def market_all_stocks(request: Request):
    ...
    try:
        ...
        total, rows = get_all_stocks(
            limit=page_size, offset=offset, search=search if search else None
        )
...
def search_stocks_api(
    ...
    max_mktcap: float = None,
    sort_by: str = Query(
        "change_pct",
        description="排序字段: change_pct|turnover|volume|per|pb|mktcap|code|name|price",
    ),
