...
async def get_limit_up():
    def fetch():
        ...
        try:
            ...
            df_work["symbol"] = df_work["code"].apply(
            ...
            df_work["change_pct"] = df_work["涨跌幅"].apply(
                lambda x: float(x) if x else 0
            )
            df_work["turnover"] = df_work["成交额"].apply(
                lambda x: float(x) if x else 0
            )
            df_work["float_market_cap"] = df_work["流通市值"].apply(
                lambda x: float(x) if x else 0
            )
            df_work["total_market_cap"] = df_work["总市值"].apply(
                lambda x: float(x) if x else 0
            )
            df_work["turnover_rate"] = df_work["换手率"].apply(
                lambda x: float(x) if x else 0
            )
            df_work["seal_fund"] = (
                df_work.get("封板资金", 0).apply(lambda x: float(x) if x else 0)
                if "封板资金" in df_work
                else 0
            )
            df_work["first_seal_time"] = df_work["首次封板时间"].astype(str)
            df_work["last_seal_time"] = (
                df_work.get("最后封板时间", "").astype(str)
                if "最后封板时间" in df_work
                else ""
            )
            df_work["board_count"] = df_work["连板数"].apply(
...
async def get_limit_down():
    def fetch():
        ...
        try:
            ...
            df_work["continue_stop"] = df_work["连续跌停"].apply(
                lambda x: int(x) if x else 0
            )
            df_work["open_count"] = (
                df_work.get("开板次数", 0).apply(lambda x: int(x) if x else 0)
                if "开板次数" in df_work
                else 0
            )
            df_work["industry"] = df_work["所属行业"].astype(str)
...
async def get_unusual():
    def fetch():
        ...
        try:
            ...
            df_filtered = df_work[
                (df_work["turnover_rate"] > 5) | (df_work["change_pct"] >= 9.5)
            ]
            df_filtered["code"] = df_filtered["代码"].astype(str)
            df_filtered["symbol"] = df_filtered["code"].apply(
                lambda x: "sh" + x if x.startswith("6") else "sz" + x
            )
            df_filtered["name"] = df_filtered["名称"].astype(str)
            df_filtered["price"] = df_filtered["最新价"].apply(
                lambda x: float(x) if x else 0
            )
            df_filtered["turnover"] = df_filtered["成交额"].apply(
                lambda x: float(x) if x else 0
            )
            df_filtered["volume_ratio"] = df_filtered["turnover_rate"] / 3
            df_filtered["industry"] = df_filtered["所属行业"].astype(str)
            df_filtered["board_count"] = df_filtered["连板数"].apply(
                lambda x: int(x) if x else 0
            )
...
def _load_stock_cache():
    ...
    try:
        ...
        stocks = fetch_all_stocks_sina(
            page_size=500, max_pages=20, timeout=15, exclude_bj=True, delay=0.3
        )
        ...
        if stocks and len(stocks) > 100:
            ...
            logger.info(
                f"[Stocks] Stock cache loaded from Sina: {len(_STOCK_CACHE)} stocks"
            )
        else:
            logger.warning(
                f"[Stocks] Sina fetch returned {len(stocks) if stocks else 0} stocks, using fallback"
            )
            _STOCK_CACHE = _COMMON_STOCKS.copy()
            logger.info(
                f"[Stocks] Stock cache loaded from fallback: {len(_STOCK_CACHE)} stocks"
            )
...
async def get_all_stocks(
    ...
    try:
        ...
        total, stocks = get_all_stocks(
            limit=limit, offset=offset, search=q if q else None
        )
            ...
            try:
                ...
                try:
                    ...
                    if profile_df is not None and len(profile_df) > 0:
                        ...
                        if (
                            not result.get("business")
                            or result["business"] == "暂无主营业务数据"
                        ):
                            result["business"] = (
                                profile.get("主营业务", "暂无主营业务数据")
                                or "暂无主营业务数据"
                            )
                        if not result.get("listDate"):
                            result["listDate"] = (
                                str(profile.get("上市日期", ""))
                                if profile.get("上市日期")
                                else None
                            )
                except Exception as e:
                    logger.debug(
                        f"[Stocks] stock_profile_cninfo error for {bare_symbol}: {e}"
                    )

                try:
                    exchange = "SH" if bare_symbol.startswith("6") else "SZ"
                    spot_df = ak.stock_individual_spot_xq(
                        symbol=f"{exchange}{bare_symbol}"
                    )
                    if spot_df is not None and len(spot_df) > 0:
                        ...
                        if float_shares:
                            try:
                                ...
                                logger.debug(
                                    f"[Stocks] Failed to parse floatShares for {bare_symbol}: {e}"
                                )
                        if float_market_cap:
                            try:
                                ...
                                logger.debug(
                                    f"[Stocks] Failed to parse floatMarketCap for {bare_symbol}: {e}"
                                )
                except Exception as e:
                    logger.debug(
                        f"[Stocks] stock_individual_spot_xq error for {bare_symbol}: {e}"
                    )
                ...
                if not result.get("totalShares"):
                    try:
                        ...
                        share_df = ak.stock_share_change_cninfo(
                            symbol=bare_symbol, start_date=one_year_ago, end_date=today
                        )
                        if (
                            share_df is not None
                            and len(share_df) > 0
                            and "总股本" in share_df.columns
                        ):
                            latest = share_df.sort_values(
                                "变动日期", ascending=False
                            ).iloc[0]
                        ...
                        logger.debug(
                            f"[Stocks] stock_share_change_cninfo error for {bare_symbol}: {e}"
                        )
                    ...
                    try:
                        ...
                        if holder_df is not None and len(holder_df) > 0:
                            ...
                            if pd.notna(shares) and pd.notna(ratio) and ratio > 0:
                                result["totalShares"] = float(shares) / (
                                    float(ratio) / 100
                                )
                                logger.debug(
                                    f"[Stocks] Derived totalShares from stock_main_stock_holder: {result['totalShares']}"
                                )
                    except Exception as e:
                        logger.debug(
                            f"[Stocks] stock_main_stock_holder error for {bare_symbol}: {e}"
                        )
                        ...
                        if info_df is not None and len(info_df) > 0:
                            ...
                            def parse_capital(value):
                                ...
                                try:
                                    ...
                                    if "亿" in value:
                                        return (
                                            float(value.replace("亿", "").strip())
                                            * 100000000
                                        )
                                    elif "万" in value:
                                        return (
                                            float(value.replace("万", "").strip())
                                            * 10000
                                        )
                                    else:
                                        return float(value)
                            ...
                            if not result.get("totalShares"):
                                result["totalShares"] = parse_capital(
                                    info_dict.get("总股本", None)
                                )
                            if not result.get("floatShares"):
                                result["floatShares"] = parse_capital(
                                    info_dict.get("流通股", None)
                                )

                            if result["price"] > 0:
                                if result.get("totalShares") and not result.get(
                                    "totalMarketCap"
                                ):
                                    result["totalMarketCap"] = (
                                        result["price"] * result["totalShares"]
                                    )
                                if result.get("floatShares") and not result.get(
                                    "floatMarketCap"
                                ):
                                    result["floatMarketCap"] = (
                                        result["price"] * result["floatShares"]
                                    )
                    except Exception as e:
                        logger.debug(
                            f"[Stocks] stock_individual_info_em error for {bare_symbol}: {e}"
                        )
                ...
                "market_sentiment": (
                    ...
                    if zt_count > 50
                        ...
                        if zt_count > 30
                            ...
                            "偏强"
                            if zt_count > 15
                            else "偏弱" if zt_count > 5 else "极弱"
                        )
                    )
                ),
                "zt_industry": dict(
                    sorted(industry_dist.items(), key=lambda x: x[1], reverse=True)[:5]
                ),
                "strongest": [
                    {
                        "name": str(
                            r.get("名称") or r.get("name") or r.get("股票名称", "")
                        ),
                        "turnover_rate": float(
                            r.get("换手率")
                            or r.get("换手率(%)")
                            or r.get("turnover_rate", 0)
                        ),
                        "change_pct": float(
                            r.get("涨跌幅") or r.get("涨跌额") or r.get("change_pct", 0)
                        ),
                    }
                    for r in (
                        strong_df.to_dict("records") if strong_df is not None else []
                    )
                ],
            }
        except Exception as e:
