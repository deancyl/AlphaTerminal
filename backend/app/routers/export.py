...
async def export_backtest_result(
    ...
    try:
        ...
        if not result:
            raise HTTPException(
                status_code=400, detail="No backtest result data provided"
            )
        ...
        data.append(
            {
                ...
                "年化收益率(%)": (
                    round(result.get("annualized_return_pct", 0) * 100, 2)
                    if result.get("annualized_return_pct")
                    else 0
                ),
                "最大回撤(%)": (
                    round(result.get("max_drawdown_pct", 0) * 100, 2)
                    if result.get("max_drawdown_pct")
                    else 0
                ),
                "夏普比率": (
                    round(result.get("sharpe_ratio", 0), 2)
                    if result.get("sharpe_ratio")
                    else 0
                ),
                "胜率(%)": (
                    round(result.get("win_rate", 0) * 100, 2)
                    if result.get("win_rate")
                    else 0
                ),
                "交易次数": result.get("trades_count", 0),
                "基准收益率(%)": (
                    round(result.get("benchmark_return_pct", 0) * 100, 2)
                    if result.get("benchmark_return_pct")
                    else 0
                ),
                "导出时间": datetime.now().isoformat(),
            }
