...
class PerformanceMetricsCalculator:
    ...
    def __init__(self, risk_free_rate: float = 0.02, trading_days: int = 252):
        ...
        logger.debug(
            f"[Cycle 0] PerformanceMetricsCalculator initialized - rf_rate={risk_free_rate}, trading_days={trading_days}"
        )

    def _validate_inputs(
        self, equity_curve: pd.Series, trades: Optional[List[Dict]] = None
    ) -> Tuple[bool, str]:
        """
        ...
        logger.debug(
            f"[Cycle 1] Input validation passed - equity_points={len(equity_curve)}"
        )
        return True, ""
    ...
    def calculate_returns(self, equity_curve: pd.Series) -> pd.Series:
        ...
        if len(equity_clean) < 2:
            logger.warning(
                "[Cycle 2] Insufficient data after cleaning, returning empty series"
            )
        ...
        logger.debug(
            f"[Cycle 2] Returns calculated - count={len(returns)}, "
    ...
    def calculate_sortino_ratio(self, returns: pd.Series) -> float:
        ...
        if len(negative_returns) == 0:
            logger.debug(
                "[Cycle 4] No negative returns, Sortino = infinite (returning high value)"
            )
    ...
    def calculate_max_drawdown(self, equity_curve: pd.Series) -> Dict[str, float]:
        ...
        return {"max_drawdown": max_dd, "drawdown_duration": max_duration}
    ...
    def calculate_win_rate(self, trades: List[Dict]) -> float:
        ...
        logger.debug(
            f"[Cycle 6] Win rate: {win_rate:.4f} ({winning_trades}/{total_trades} trades)"
        )
    ...
    def calculate_profit_factor(self, trades: List[Dict]) -> float:
        ...
        logger.debug(
            f"[Cycle 7] Gross profit: {gross_profit:.2f}, Gross loss: {gross_loss:.2f}"
        )
    ...
    def calculate_var(self, returns: pd.Series, confidence: float = 0.95) -> float:
        ...
        """
        logger.debug(
            f"[Cycle 8 - VaR] Starting VaR calculation at {confidence*100}% confidence"
        )
    ...
    def calculate_cvar(self, returns: pd.Series, confidence: float = 0.95) -> float:
        ...
        """
        logger.debug(
            f"[Cycle 8 - CVaR] Starting CVaR calculation at {confidence*100}% confidence"
        )
    ...
    def analyze_trades(self, trades: List[Dict]) -> Dict[str, Any]:
        ...
        logger.debug(
            f"[Cycle 9] Trade analysis complete - {analysis['total_trades']} trades analyzed"
        )
    ...
    def _calculate_trade_distribution(self, pnls: List[float]) -> Dict[str, int]:
        ...
        distribution = {
            "large_wins": (
                sum(1 for p in pnls if p > np.mean([x for x in pnls if x > 0]) * 2)
                if any(p > 0 for p in pnls)
                else 0
            ),
            "small_wins": (
                sum(1 for p in pnls if 0 < p <= np.mean([x for x in pnls if x > 0]))
                if any(p > 0 for p in pnls)
                else 0
            ),
            "small_losses": (
                sum(1 for p in pnls if np.mean([x for x in pnls if x < 0]) <= p < 0)
                if any(p < 0 for p in pnls)
                else 0
            ),
            "large_losses": (
    ...
    def calculate_all_metrics(
        ...
        """
        logger.debug(
            "[Cycle 10 - Metrics Aggregation] Starting comprehensive metrics calculation"
        )
        ...
        logger.debug(
            f"[Cycle 10] Metrics aggregation complete - {len(metrics)} metrics calculated"
        )
...
class PerformanceAnalyzer:
    ...
    def calculate_tearsheet_metrics(
        ...
        try:
            ...
            logger.error(
                f"[PerformanceAnalyzer] Error calculating metrics: {e}", exc_info=True
            )
            return {
                "code": 1,
    ...
    def _calculate_all_metrics(
        self, returns: pd.Series, benchmark_returns: Optional[List[float]] = None
    ) -> Dict[str, Any]:
        """Calculate all performance metrics"""
        ...
        try:
            ...
            logger.warning(
                "pyfolio-reloaded not installed, using fallback calculations",
                exc_info=True,
            )
        ...
        try:
            ...
            if benchmark_returns and len(benchmark_returns) == len(returns):
                ...
                metrics["excess_return"] = (
                    metrics["annual_return"] - metrics["benchmark"]["annual_return"]
                )
            ...
            logger.warning(
                f"pyfolio calculation failed: {e}, using fallback", exc_info=True
            )
            return self._fallback_metrics(returns)
    ...
    def _fallback_metrics(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        downside_std = (
            float(downside_returns.std() * np.sqrt(252))
    ...
    def calculate_from_equity_curve(
        ...
        try:
            ...
            return self.calculate_tearsheet_metrics(
                returns=returns, dates=dates[1:]  # Returns start from second day
            )

        except Exception as e:
            logger.error(
                f"[PerformanceAnalyzer] Error processing equity curve: {e}",
                exc_info=True,
            )
            return {
                "code": 1,
        ...
        try:
            ...
            for trade in trades:
                if "pnl" in trade:
                    ...
                    equity_curve.append({"date": date, "value": equity})
            ...
            logger.error(
                f"[PerformanceAnalyzer] Error processing trades: {e}", exc_info=True
            )
            return {
                "code": 1,
