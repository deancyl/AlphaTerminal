...
def acquire_scheduler_lock():
    ...
    try:
        ...
        logger.info(
            "[Scheduler] Acquired exclusive lock - this worker will run polling jobs"
        )
        return True
    except (IOError, OSError):
        logger.info(
            "[Scheduler] Another worker holds the lock - this worker will skip polling jobs"
        )
        if _lock_file:
            _lock_file.close()
            _lock_file = None
        ...
        def _sync_broadcast():
            ...
            try:
                for r in rows:
                    ...
                    prev = (
                        price / (1 + change_pct / 100) if change_pct != -100 else price
                    )
                ...
                logger.debug(
                    f"[WS-Broadcast] 推送 {len(rows)} 条 tick 到 {ws_manager.total()} 个连接"
                )
            finally:
                loop.close()
...
def backfill_daily_history():
    ...
    for sym in HISTORY_SYMBOLS:
        try:
            ...
            logger.info(
                f"[Scheduler] Backfill: Symbol {sym} success, items={len(rows)}"
            )
        except Exception as e:
            logger.error(
                f"[Scheduler] Backfill: Symbol {sym} failed: {e}", exc_info=True
            )
...
def start_scheduler():
    ...
    def _refresh_today_daily():
        ...
        for sym in ["000001", "000300", "399001", "399006", "000688"]:
            try:
                rows = fetch_china_index_history(sym)
                logger.debug(
                    f"[Scheduler] 今日K线 {sym}: {len(rows)} rows, latest={rows[-1]['date'] if rows else 'N/A'}"
                )
            except Exception as e:
                logger.warning(
                    f"[Scheduler] 今日K线 {sym} 刷新失败: {e}", exc_info=True
                )
    ...
    def _initial_all_stocks():
        ...
        try:
            ...
            if count < 100:
                logger.error(
                    f"[Scheduler] CRITICAL: Only {count} stocks loaded, expected 5000+"
                )
                logger.error(
                    "[Scheduler] Check Sina API connectivity and circuit breaker status"
                )
            else:
    ...
    def _forex_polling_job():
        ...
        try:
                ...
                for base_curr in currency_list:
                    ...
                    for quote_curr in currency_list:
                        if base_curr == quote_curr:
                            ...
                            rate = forex_fetcher.calculate_cross_rate(
                                base_curr, quote_curr, rates_dict
                            )
                            ...
                            row_rates.append(
                                {
                                    ...
                                    "is_base": False,
                                    "is_calculated": (
                                        direct_key not in rates_dict if rate else False
                                    ),
                                }
                            )
            ...
            logger.info(
                f"[Scheduler] Forex polling complete: {len(quotes) if quotes else 0} quotes, matrix {len(matrix)}x{len(matrix[0]['rates']) if matrix else 0}"
            )
        except Exception as e:
    ...
    def _record_portfolio_snapshots():
        ...
        try:
            ...
            for (pid,) in rows:
                ...
                logger.info(
                    f"[PortfolioSnap] pid={pid} date={today} asset={result.get('total_asset',0)}"
                )
        except Exception as e:
    ...
    try:
        ...
        for q in quotes:
            ...
            if symbol.endswith("CNY"):
                rates_dict[symbol.replace("CNY", "")] = Decimal(
                    str(q.get("bid", q.get("price", 0)))
                )
        ...
        for item in cfets_rmb:
            ...
            if symbol.endswith("CNY"):
                rates_dict[symbol.replace("CNY", "")] = Decimal(
                    str(item.get("bid", item.get("price", 0)))
                )

        cache.set("forex:spot:v1", quotes, ttl=60)
        cache.set(
            "forex:rates:v1", {k: float(v) for k, v in rates_dict.items()}, ttl=60
        )
...
def _memory_monitor():
    ...
    try:
        ...
        if mem_mb > MEMORY_THRESHOLD_MB:
            logger.warning(
                f"[MemoryMonitor] 内存使用 {mem_mb:.1f}MB 超过阈值 {MEMORY_THRESHOLD_MB}MB，执行强制gc"
            )
            gc.collect()
            # 再次检查
            new_mem_mb = process.memory_info().rss / 1024 / 1024
            logger.info(
                f"[MemoryMonitor] 强制gc后内存: {new_mem_mb:.1f}MB (释放 {mem_mb - new_mem_mb:.1f}MB)"
            )
        else:
...
def _macro_polling_job():
    ...
    try:
            ...
            for old_col, new_col in rename_map.items():
                if old_col in df_work.columns:
                    if new_col in ("month", "quarter"):
                        df_work[new_col] = df_work[old_col].apply(
                            lambda x: str(x) if x is not None else None
                        )
                    else:
                        df_work[new_col] = df_work[old_col].apply(_safe_float)
            df_work = df_work.drop(
                columns=[c for c in columns if c not in rename_map.values()]
            )
        ...
        dashboard["overview"] = {
            "gdp": {
                "quarter": (
                    _safe_strftime(latest_gdp["季度"])
                    if latest_gdp is not None
                    else None
                ),
                "value": (
                    _safe_float(latest_gdp["国内生产总值-绝对值"])
                    if latest_gdp is not None
                    else None
                ),
                "yoy": (
                    _safe_float(latest_gdp["国内生产总值-同比增长"])
                    if latest_gdp is not None
                    else None
                ),
            },
            "cpi": {
                "month": (
                    _safe_strftime(latest_cpi["月份"], "%Y年%m月")
                    if latest_cpi is not None
                    else None
                ),
                "yoy": (
                    _safe_float(latest_cpi["全国-同比增长"])
                    if latest_cpi is not None
                    else None
                ),
                "mom": (
                    _safe_float(latest_cpi["全国-环比增长"])
                    if latest_cpi is not None
                    else None
                ),
            },
            "ppi": {
                "month": (
                    _safe_strftime(latest_ppi["月份"], "%Y年%m月")
                    if latest_ppi is not None
                    else None
                ),
                "yoy": (
                    _safe_float(latest_ppi["当月同比增长"])
                    if latest_ppi is not None
                    else None
                ),
            },
            "pmi": {
                ...
                "value": (
                    _safe_float(latest_pmi["今值"]) if latest_pmi is not None else None
                ),
            },
            "m2": {
                "month": str(latest_m2["日期"]) if latest_m2 is not None else None,
                "yoy": (
                    _safe_float(latest_m2["今值"]) if latest_m2 is not None else None
                ),
            },
        }
                ...
                calendar_items.append(
                    {
                        ...
                        "importance": (
                            "high" if "重要" in str(row.get("重要性", "")) else "normal"
                        ),
                    }
                )
        ...
        dashboard["m2"] = {
            "data": process_df(
                results["m2"], 24, ["日期", "今值"], {"日期": "month", "今值": "m2_yoy"}
            ),
            "unit": "%",
            "frequency": "月度",
        }
        ...
        logger.info(
            f"[MacroPolling] Complete, cached macro:dashboard with {len(dashboard)} indicators"
        )
