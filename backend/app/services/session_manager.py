...
class SessionManager:
    ...
    def _cleanup_loop(self):
        ...
        while not self._shutdown:
            try:
                ...
                if deleted > 0:
                    logger.debug(
                        f"[SessionManager] Cleaned up {deleted} expired sessions"
                    )
    ...
    def update_session_usage(
        ...
        return session_db.update_session_stats(
            session_id, tokens_added=tokens, cost_added=cost_usd
        )
    ...
    def extend_session(self, session_id: str, additional_minutes: int = 30) -> bool:
        ...
        return session_db.extend_session(
            session_id, additional_hours=additional_minutes // 60 or 1
        )
