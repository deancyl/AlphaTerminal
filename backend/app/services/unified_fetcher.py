...
class UnifiedFetcher:
    ...
    async def fetch(
        ...
        if cached is not None:
            ...
            return FetchResult(
                data=cached, source=source, latency_ms=latency_ms, from_cache=True
            )
        ...
        try:
            ...
            logger.warning(
                f"[UnifiedFetcher] 熔断器打开: {source.value}, {e}", exc_info=True
            )
        except Exception as e:
            logger.error(
                f"[UnifiedFetcher] 主数据源失败: {source.value}, {e}", exc_info=True
            )
        ...
        if fallback_fn is not None:
            ...
            try:
                if fallback_breaker.is_available():
                    with fallback_breaker:
                        ...
                        self.metrics.record_latency(
                            fallback_source.value, latency_ms / 1000
                        )

                        logger.info(
                            f"[UnifiedFetcher] 降级数据源成功: {fallback_source.value}"
                        )
                ...
                logger.warning(
                    f"[UnifiedFetcher] 降级熔断器打开: {fallback_source.value}, {e}",
                    exc_info=True,
                )
            except Exception as e:
                logger.error(
                    f"[UnifiedFetcher] 降级数据源失败: {fallback_source.value}, {e}",
                    exc_info=True,
                )
    ...
    async def _execute_fetch(
        self, fetch_fn: Union[Callable[[], Any], Callable[[], Awaitable[Any]]]
    ) -> Any:
        """
        执行获取函数（支持同步和异步）
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "breakers": {},
            "metrics": {"hit_rate": self.metrics.get_hit_rate(), "sources": {}},
        }
