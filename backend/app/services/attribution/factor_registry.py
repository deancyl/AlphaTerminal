...
class FactorRegistry:
    ...
    def list_categories(self) -> List[dict]:
        ...
        category_info = {
            FactorCategory.VALUE: {"id": "value", "name": "价值因子", "icon": "💰", "description": "估值相关指标"},
            FactorCategory.GROWTH: {"id": "growth", "name": "成长因子", "icon": "📈", "description": "增长相关指标"},
            FactorCategory.QUALITY: {"id": "quality", "name": "质量因子", "icon": "⭐", "description": "盈利质量指标"},
            FactorCategory.MOMENTUM: {"id": "momentum", "name": "动量因子", "icon": "🚀", "description": "价格动量指标"},
    ...
    def calculate(self, factor_id: str, data: pd.DataFrame) -> Optional[Union[np.ndarray, Any]]:
        """
    ...
    def _calc_macd(self, data: pd.DataFrame, fast: int = 12, slow: int = 26, signal: int = 9) -> np.ndarray:
        """Calculate MACD"""
        if 'close' not in data.columns:
            return np.full(len(data), np.nan)
    ...
    def _calc_boll(self, data: pd.DataFrame, period: int = 20, std_dev: float = 2) -> np.ndarray:
