...
from app.services.copilot.data_fetchers.f9_fetcher import get_f9_fetcher
from app.services.copilot.data_fetchers.macro_fetcher import get_macro_fetcher
from app.services.copilot.data_fetchers.news_fetcher import get_news_fetcher
...
class ContextAssembler:
    ...
    async def assemble(
        ...
        logger.info(
            f"[ContextAssembler] 查询分类: {query_type.value}, symbols: {symbols}"
        )
        ...
        try:
            result = await asyncio.wait_for(
                self._fetch_all_data(config, symbols, portfolio_id), timeout=timeout
            )
    ...
    async def _fetch_all_data(
        self, config: ContextConfig, symbols: List[str], portfolio_id: Optional[int]
    ) -> Dict[str, Any]:
        """
        并行获取所有数据
        ...
        if config.news_days > 0 and symbols:
            tasks.append(
                self._news_fetcher.fetch(symbol=symbols[0], days=config.news_days)
            )
    ...
    async def _fetch_portfolio_data(
        ...
        try:
            ...
            logger.warning(
                f"[ContextAssembler] 投资组合数据获取失败: {e}", exc_info=True
            )
            return None
    ...
    def _estimate_tokens(self, text: str) -> int:
        ...
        # 统计其他字符
        other_chars = (
            len(text) - chinese_chars - sum(len(w) for w in text.split() if w.isascii())
        )
