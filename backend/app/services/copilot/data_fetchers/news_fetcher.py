...
class NewsFetcher:
    ...
    async def fetch(
        ...
        try:
            ...
            return NewsDataResult(news_items=[], error=f"新闻获取超时 ({timeout}秒)")
        except Exception as e:
            logger.error(f"[NewsFetcher] 获取失败: {e}", exc_info=True)
            return NewsDataResult(news_items=[], error=f"新闻获取失败: {str(e)}")

    async def _fetch_general_news(self, limit: int, timeout: float) -> NewsDataResult:
        """
        获取通用市场新闻
        ...
        try:
            ...
            if not is_cache_ready():
                ...
                return NewsDataResult(
                    news_items=[], sentiment="neutral", error="新闻缓存未就绪"
                )
            ...
            if not news:
                return NewsDataResult(news_items=[], sentiment="neutral")
    ...
    async def _fetch_symbol_news(
        self, symbol: str, days: int, limit: int, timeout: float
    ) -> NewsDataResult:
        """
        获取特定股票的新闻
        ...
        try:
            ...
            response = await asyncio.wait_for(
                news_router.news_events_for_symbol(symbol, limit=limit), timeout=timeout
            )
            ...
            logger.info(
                f"[NewsFetcher] 获取 {symbol} 新闻成功: {len(formatted_news)} 条"
            )
