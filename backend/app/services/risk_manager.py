...
class RiskManager:
    ...
    def __init__(self, config: Optional[RiskConfig] = None):
        ...
        logger.info("[RiskManager] Initialized with config:")
    ...
    def update_trailing_stop(
        ...
        if not self.config.trailing_stop_enabled:
            logger.debug("  [SKIPPED] Trailing stop disabled in config")
            logger.debug("=" * 60)
    ...
    def check_stop_triggered(
        ...
        if triggered:
            ...
            logger.debug("  [RESULT] No stop triggered")
    ...
    def check_profit_target(
        ...
        if not position.target_price or position.target_price <= 0:
            logger.debug("  [SKIPPED] No target price set")
            logger.debug("=" * 60)
            return False
    ...
    def adjust_risk_after_loss(
        ...
        if consecutive_losses <= 0:
            logger.debug("  [NO CHANGE] No consecutive losses")
            logger.debug("=" * 60)
