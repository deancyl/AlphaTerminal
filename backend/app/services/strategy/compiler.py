...
if not logger.handlers:
    ...
    formatter = logging.Formatter(
        "[%(asctime)s] [%(levelname)s] [StrategyCompiler] %(message)s"
    )
...
class StrategyCompiler:
    ...
    def compile(self, code: str, strategy_type: str = "auto") -> CompilationResult:
        ...
        try:
            ...
            logger.info(
                f"[SUCCESS] Strategy '{spec.name}' compiled in {result.compilation_time_ms:.2f}ms"
            )
    ...
    def _debug_cycle_1_input_validation(self, code: str) -> Dict[str, Any]:
        ...
        logger.debug(
            f"  Check 1: Code not empty -> {'PASS' if check['passed'] else 'FAIL'}"
        )

        # Check 2: Minimum length
        min_length = 20
        check = {
            "name": f"Minimum length ({min_length} chars)",
            "passed": len(code.strip()) >= min_length,
        }
        if not check["passed"]:
            check["error"] = f"Code too short: {len(code.strip())} chars"
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 2: Minimum length -> {'PASS' if check['passed'] else 'FAIL'}"
        )

        # Check 3: Contains annotations or code structure
        has_annotations = bool(
            self.NAME_PATTERN.search(code)
            or self.DESC_PATTERN.search(code)
            or self.PARAM_PATTERN.search(code)
            or self.STRATEGY_PATTERN.search(code)
        )
        has_code_structure = "def " in code or "output" in code or "=" in code
        check = {
            "name": "Has valid structure",
            "passed": has_annotations or has_code_structure,
        }
        if not check["passed"]:
            check["error"] = "No annotations or code structure found"
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 3: Valid structure -> {'PASS' if check['passed'] else 'FAIL'}"
        )
    ...
    def _debug_cycle_2_annotation_parsing(
        ...
        logger.debug(
            f"  Description: {spec.description[:50]}..."
            if len(spec.description) > 50
            else f"  Description: {spec.description}"
        )
        ...
        logger.debug(
            f"  Result: {'PASS' if cycle_data['passed'] else 'PASS (with warnings)'}"
        )
    ...
    def _debug_cycle_4_strategy_config_extraction(
        ...
        logger.debug(
            f"  Trailing Stop: {'Enabled' if spec.trailing_stop_enabled else 'Disabled'}"
        )
    ...
    def _debug_cycle_5_code_validation(self, code: str) -> Tuple[bool, List[str]]:
        ...
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 2: Output definition -> {'PASS' if has_output else 'FAIL'}"
        )
    ...
    def _debug_cycle_6_security_checks(self, code: str) -> Tuple[bool, List[str]]:
        ...
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 1: Forbidden imports -> {'PASS' if not has_forbidden_imports else 'FAIL'}"
        )

        # Check 2: Forbidden builtins
        has_forbidden_builtins = any(
            f"{builtin}(" in code for builtin in FORBIDDEN_BUILTINS
        )
        check = {"name": "No forbidden builtins", "passed": not has_forbidden_builtins}
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 2: Forbidden builtins -> {'PASS' if not has_forbidden_builtins else 'FAIL'}"
        )
        ...
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 3: File operations -> {'PASS' if not has_file_ops else 'FAIL'}"
        )
        ...
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 4: Network calls -> {'PASS' if not has_network else 'FAIL'}"
        )
        ...
        cycle_data["checks"].append(check)
        logger.debug(
            f"  Check 5: eval/exec -> {'PASS' if not has_eval_exec else 'FAIL'}"
        )
    ...
    def _debug_cycle_7_indicator_compilation(
        ...
        cycle_data["steps"].append(
            {"name": "Code extraction", "lines": len(code_lines)}
        )
        ...
        cycle_data["steps"].append(
            {"name": "Namespace creation", "symbols": len(namespace)}
        )
    ...
    def _debug_cycle_9_execution_test(
        ...
        try:
            ...
            cycle_data["tests"].append(
                {"name": "Function callable", "passed": False, "error": str(e)}
            )
            cycle_data["passed"] = False
        ...
        if cycle_data["passed"]:
            ...
            try:
                ...
                cycle_data["tests"].append(
                    {"name": "Execution test", "passed": False, "error": str(e)}
                )
                cycle_data["passed"] = False
    ...
    def _debug_cycle_10_summary(self, result: CompilationResult) -> None:
        ...
        logger.debug(
            f"  Type: {result.spec.strategy_type if result.spec else 'Unknown'}"
        )
    ...
    def _compile_indicator_strategy(self, code: str, spec: StrategySpec) -> Callable:
        ...
        def execute_strategy(
            ...
            try:
                ...
                logger.warning(
                    f"[IndicatorStrategy] Execution error: {e}", exc_info=True
                )
                return {
                    "indicators": {},
                    "signals": {
    ...
    def _compile_script_strategy(self, code: str, spec: StrategySpec) -> Callable:
        """Compile script strategy to executable function."""
        from .script_strategy import ScriptStrategy
...
def compile_strategy(
    code: str, strategy_type: str = "auto", debug_level: int = 10
) -> CompilationResult:
    """
    Compile strategy code to executable function.
