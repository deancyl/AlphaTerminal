            ...
            if score > 0.2
                ...
                if score > 0.05
                    ...
                    if score > -0.05
                        ...
                        "中性偏空"
                        if score > -0.2
                        else "偏利空" if score > -0.6 else "极度利空"
                    )
                )
            )
        )
...
def _bg_sina_refresh():
    ...
    try:
        ...
        if rows and len(rows) >= 5:
            ...
            logger.info(
                f"[HEARTBEAT] Spot cache (Sina HQ): {len(rows)} stocks from pool of {len(pool)}"
            )
            return
        logger.warning("[SpotCache] Sina HQ 返回不足，尝试从数据库兜底")
    except Exception as e:
        logger.warning(
            f"[SpotCache] Sina HQ 失败，尝试从数据库兜底: {type(e).__name__}: {e}",
            exc_info=True,
        )
                ...
                rows.append(
                    {
                        ...
                        "turnover": float(r.get("turnover") or 0),
                        "volume": float(r.get("volume") or 0),
                        "market": r.get("code", "")[:2] in ("60", "68", "90")
                        and "SH"
                        or "SZ",
                    }
                )
    ...
    def _fetch_and_analyze():
        ...
        try:
            ...
            if cached:
                ...
                logger.info(
                    f"[HEARTBEAT] News+Sentiment refreshed: {len(cached)} items"
                )
            else:
                logger.warning("[News] 缓存为空，跳过情感分析")
        except Exception as e:
            logger.error(
                f"[News] 刷新+情感分析失败: {type(e).__name__}: {e}", exc_info=True
            )

    t = threading.Thread(
        target=_fetch_and_analyze, daemon=True, name="news-sentiment-fetch"
    )
    t.start()
