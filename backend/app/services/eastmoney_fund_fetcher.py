...
class EastmoneyFundFetcher:
        ...
        try:
            async with self._get_client() as client:
                ...
                js_resp, html_resp = await asyncio.gather(
                    client.get(js_url), client.get(html_url), return_exceptions=True
                )
    ...
    def _parse_html_info(self, html: str, result: Dict) -> None:
        ...
        try:
            # 提取基金经理 - 找基金经理标签后面的名字
            # 模式：基金经理...>名字</a>
            manager_matches = re.findall(
                r"基金经理[\s\S]{0,200}?>\s*([^<]{2,8}?)\s*</a>", html
            )
    ...
    def _parse_pingzhong_data(self, js_text: str, code: str) -> Optional[Dict]:
        ...
        try:
            ...
            # 从净值走势数据中提取最新净值
            nav_trend_match = re.search(
                r"Data_netWorthTrend\s*=\s*(\[.*?\]);", js_text, re.DOTALL
            )
            if nav_trend_match:
                try:
                    ...
                    if nav_data and len(nav_data) > 0:
                        ...
                        if len(nav_data) > 1:
                            ...
                            if prev_nav and result["nav"]:
                                result["nav_change_pct"] = round(
                                    (result["nav"] - prev_nav) / prev_nav * 100, 2
                                )
                except Exception as e:
            ...
            acc_nav_match = re.search(
                r"Data_ACWorthTrend\s*=\s*(\[.*?\]);", js_text, re.DOTALL
            )
            if acc_nav_match:
                try:
            ...
            async with self._get_client() as client:
                ...
                for i, item in enumerate(nav_data[-limit:]):
                    ...
                    if ts and nav:
                        result.append(
                            {
                                "date": datetime.fromtimestamp(ts / 1000).strftime(
                                    "%Y-%m-%d"
                                ),
                                "nav": nav,
                ...
                logger.info(
                    f"[Eastmoney] 基金 {code} 净值历史获取成功，共 {len(result)} 条"
                )
                return result

        except Exception as e:
            logger.warning(
                f"[Eastmoney] 基金 {code} 净值历史获取失败: {e}", exc_info=True
            )
    ...
    async def get_fund_portfolio(self, code: str) -> Optional[Dict]:
        ...
        try:
            ...
            df = await asyncio.wait_for(
                asyncio.to_thread(ak.fund_portfolio_hold_em, symbol=code), timeout=15.0
            )
            ...
            if df is not None and not df.empty:
                ...
                logger.info(
                    f"[Eastmoney] 基金 {code} 持仓获取成功（akshare），{len(stocks)} 只重仓股"
                )
            ...
            logger.warning(
                f"[Eastmoney] 基金 {code} 持仓 akshare 超时，降级到 pingzhongdata.js",
                exc_info=True,
            )
        except Exception as e:
            logger.warning(
                f"[Eastmoney] 基金 {code} 持仓 akshare 失败: {e}，降级到 pingzhongdata.js",
                exc_info=True,
            )
    ...
    async def _get_portfolio_from_pingzhongdata(self, code: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with self._get_client() as client:
                ...
                stock_codes_match = re.search(
                    r"stockCodesNew\s*=\s*(\[[^\]]*\])", js_text
                )
                if not stock_codes_match:
                    stock_codes_match = re.search(
                        r"stockCodes\s*=\s*(\[[^\]]*\])", js_text
                    )
                ...
                if stock_codes_match:
                    ...
                    logger.info(
                        f"[Eastmoney] 基金 {code} 持仓获取成功（pingzhongdata），{len(stocks)} 只重仓股（无比例）"
                    )
            ...
            logger.warning(
                f"[Eastmoney] 基金 {code} 持仓 pingzhongdata 失败: {e}", exc_info=True
            )
    ...
    async def get_fund_returns(self, code: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(
                f"[Eastmoney] 基金 {code} 阶段收益获取失败: {e}", exc_info=True
            )
