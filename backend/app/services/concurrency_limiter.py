...
class ConcurrencyLimiter:
    ...
    def _ensure_semaphore(
        self, model_key: str, max_concurrent: int = DEFAULT_MAX_CONCURRENT
    ):
        ...
        with self._lock:
            if model_key not in self._semaphores:
                ...
                self._metrics[model_key] = ModelMetrics(
                    model_key=model_key, max_concurrent=max_concurrent
                )

    async def acquire(
        self, provider: str, model_id: str, timeout: float = DEFAULT_TIMEOUT
    ) -> bool:
        """
        ...
        try:
            ...
            logger.warning(
                f"[ConcurrencyLimiter] Timeout for {model_key}", exc_info=True
            )
            return False
    ...
    def update_limit(self, provider: str, model_id: str, new_limit: int):
        ...
        with self._lock:
            ...
            if model_key in self._metrics:
                ...
                self._metrics[model_key] = ModelMetrics(
                    model_key=model_key, max_concurrent=new_limit
                )

            logger.info(
                f"[ConcurrencyLimiter] Updated {model_key} limit to {new_limit}"
            )
...
class ConcurrencyContext:
    ...
    async def __aenter__(self) -> bool:
        self._acquired = await self._limiter.acquire(
            self._provider, self._model_id, self._timeout
        )
