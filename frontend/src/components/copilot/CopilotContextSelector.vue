...
<template>
  <!-- 上下文勾选框 -->
  <div class="px-4 py-2 border-b border-theme-secondary flex flex-wrap gap-3 text-xs shrink-0" role="group" aria-label="上下文选择">
  ...
  <!-- Provider 选择 -->
  <div class="px-4 py-2 border-b border-theme-secondary flex items-center gap-2 shrink-0" role="group" aria-label="模型选择">
  ...
  <div v-if="ctxPortfolio"
       class="px-4 py-2 border-b border-theme-secondary flex items-center gap-2 shrink-0" role="group" aria-label="投资组合选择">
    ...
    <span v-else class="flex-1 text-[11px] text-terminal-dim" role="status">暂无可用组合</span>
  </div>
</template>
