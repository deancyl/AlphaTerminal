  ...
  <div class="flex flex-col h-full min-h-0 md:min-h-0 bg-terminal-bg">
    ...
    <div class="flex items-center gap-2 mb-1 shrink-0">
      <input
        ...
        type="text"
        placeholder="代码/名称"
        class="flex-1 min-w-0 bg-terminal-bg border border-theme-secondary rounded px-2 py-0.5 text-xs text-theme-primary outline-none focus:border-terminal-accent/60"
      />
    </div>
      ...
      <table class="w-full text-xs whitespace-nowrap">
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-1.5 py-1 min-w-0 truncate text-theme-primary text-xs">{{ item.name || item.symbol }}</td>
            <td class="px-1.5 py-1 text-right font-mono text-xs text-theme-primary">{{ item.price != null ? Number(item.price).toFixed(2) : '--' }}</td>
            <td
              class="px-1.5 py-1 text-right font-mono text-xs"
    ...
    <div v-if="totalPages > 1" class="shrink-0 bg-theme-panel pb-1 flex items-center justify-center gap-1 mt-1">
      ...
      <span class="text-[10px] text-terminal-dim px-1">{{ currentPage }}/{{ totalPages }}</span>
