<template>
  ...
  >
    ...
    <div v-else-if="error && !loading" class="error-container flex flex-col items-center justify-center h-full px-4 py-8">
      <div class="text-center">
        ...
        <p class="text-sm text-theme-secondary mb-3">{{ error?.message || '数据加载失败' }}</p>
        <button
          v-if="error?.retry"
          @click="handleRetry"
          class="px-4 py-2 text-xs font-medium rounded border border-red-500/40 bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-colors"
        >
          重试
        </button>
      </div>
