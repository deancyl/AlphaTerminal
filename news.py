...
async def news_flash():
    ...
    快讯瀑布流（只读缓存，后台刷新线程维护）
    响应时间 < 50ms
    真实缓存未就绪时，Fallback 至本地 Mock 新闻（永不白屏）
    """
    ...
    # Fallback: 本地 Mock 新闻（50条，覆盖主要板块）
    mock_news = get_mock_news()
    logger.warning(f"[News] 使用 Mock Fallback，共 {len(mock_news)} 条（真实来源未就绪）")
    return {"news": mock_news, "source": "mock", "total": len(mock_news)}
